...
: "${MEILISEARCH_INDEX_UID:?MEILISEARCH_INDEX_UID is required in .env}"

URL="${MEILISEARCH_URL}/indexes/${MEILISEARCH_INDEX_UID}/search"
QUERY="counter target spell"
LIMIT=5
PAYLOAD="{\"q\":\"${QUERY}\",\"limit\":${LIMIT},\"showRankingScore\":true}"
